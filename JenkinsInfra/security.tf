resource "azurerm_user_assigned_identity" "vm" {
  ...
  name = "${module.rg.name}-${local.build_vm}-uai"
}
...
resource "azurerm_key_vault_secret" "jenkins_pass" {
  name         = "jenkins-pass"
...
resource "azurerm_role_assignment" "owner_current_subscription" {
  scope              = data.azurerm_subscription.subscription.id
  role_definition_id = "${data.azurerm_subscription.subscription.id}${data.azurerm_role_definition.owner.id}"
